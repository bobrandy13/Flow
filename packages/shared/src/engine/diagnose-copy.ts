...
export const DIAGNOSIS_COPY: Record<DiagnosisCategory, DiagnosisCopyTemplate> = {
  no_failover: {
    ...
    explanation:
      "A scheduled outage took down your {kind} for a stretch of the run. Because there was only one of them, every request that needed it during the outage was dropped. Single points of failure look fine until something fails — that's exactly the scenario this level is testing.",
    suggestions: [
  ...
  breaker_absent: {
    ...
    explanation:
      "When the {kind} got flaky, the upstream server kept sending requests at it. Each one hung waiting for a response, the server's in-flight slots filled up, and good traffic started getting dropped at the server too. The original failure didn't cause the outage — the lack of a backstop did.",
    suggestions: [
  ...
  queue_overflow: {
    ...
    explanation:
      "Producers handed work to the queue faster than the downstream consumer could pull from it. The pending depth climbed until the queue's buffer was exhausted, after which every new arrival was dropped. A queue absorbs bursts — it doesn't increase the long-run throughput of what's behind it.",
    suggestions: [
  ...
  node_overloaded: {
    ...
    explanation:
      "The {kind} hit its concurrency limit and started dropping requests. Most of the drops in this run happened there. A single instance has a ceiling on how many requests can be in flight at once — past that, new arrivals don't queue, they're rejected.",
    suggestions: [
  ...
  cache_underused: {
    headline: "Your cache isn't catching enough traffic",
    explanation:
      "There's a cache in your design, but the bottleneck is downstream of it — most requests are still falling through to slower nodes. A cache only helps if it actually intercepts the hot path before it reaches the expensive component.",
    suggestions: [
